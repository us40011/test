...
test("CARTO basemaps use MapLibre vector styles through the Leaflet layer adapter", () => {
  ...
  assert.match(html, /dark: L\.maplibreGL\(\{style:'https:\/\/basemaps\.cartocdn\.com\/gl\/dark-matter-gl-style\/style\.json'\}\)/);
  assert.match(html, /voyager: L\.maplibreGL\(\{style:'https:\/\/basemaps\.cartocdn\.com\/gl\/voyager-gl-style\/style\.json'\}\)/);
  assert.match(html, /positron: L\.maplibreGL\(\{style:'https:\/\/basemaps\.cartocdn\.com\/gl\/positron-gl-style\/style\.json'\}\)/);
