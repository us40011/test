...
// 在首次 tilemap 探测完成前，先锁定在已显示的层级。这样用户不能在“尚未确认是否有
// 影像”的时间窗口内把地图放大到灰色占位瓦片；探测成功后才逐步放开到实际可用上限。
...
function switchLayer(name) {
  ...
  currentLayer.addTo(map);
  // 卫星图层必须先完成当前位置的可用性探测，不能先短暂放开到全局 z19；否则在
  // 异步请求返回前点击“+”仍可能加载灰色无数据瓦片。其它图层维持原有上限逻辑。
  map.setMaxZoom(name === 'satellite' ? Math.min(map.getZoom(), satelliteMaxZoom) : currentLayer.options.maxZoom);
